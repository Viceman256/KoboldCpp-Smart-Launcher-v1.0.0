    ...
    try:
        if platform.system() == "Windows":
            gpu_cmd = subprocess.run(
                ...
                capture_output=True, text=True, check=False, timeout=5, creationflags=subprocess.CREATE_NO_WINDOW
            )
...
def suggested_patterns():
    ...
    patterns = [
        {"name": "MoE Models (e.g., Mixtral, some Qwen variants)","pattern": "--overridetensors \"ffn_.*_exps\\.weight=CPU\"","description": "Offloads all MoE expert FFN weights to CPU. Adjust pattern if your MoE model uses a different naming scheme for expert tensors."},
        {"name": "Dense Models - Moderate CPU Offload","pattern": "--overridetensors \"blk\\.\\d*[13579]\\.ffn_up\\.weight=CPU\"","description": "Offloads FFN up-projection weights in odd-numbered layers to CPU."},
        {"name": "Dense Models - Aggressive FFN CPU Offload","pattern": "--overridetensors \"blk\\.\\d+\\.(ffn_up|ffn_gate)\\.weight=CPU\"","description": "Offloads all FFN up-projection and gate weights to CPU."},
        {"name": "Dense Models - Targeted High-Layer FFN Offload","pattern": "--overridetensors \"blk\\.(2[4-9]|[3-9]\\d+)\\.ffn_.*\\.weight=CPU\"","description": "Example: Offloads all FFN weights in layers 24 and above to CPU."},
        {"name": "Custom Pattern Template","pattern": "--overridetensors \"LAYER_REGEX\\.TENSOR_REGEX=CPU\"","description": "Replace LAYER_REGEX (e.g., blk\\.\\d+) and TENSOR_REGEX (e.g., ffn_down\\.weight) with appropriate regex."}
    ]
    for p in patterns:
        print(f"  {BOLD}{p['name']}{ENDC}")
        print_command(f"  {p['pattern']}")
...
def main():
  ...
  All token-per-second (t/s) figures are approximate and for illustration.""")
    ...
    print_header(f"Try TensorTune v1.1.1")
    print(f"""  To use TensorTune for your own models:

  1. {BOLD}Ensure TensorTune is installed:{ENDC}
     If you haven't already, run: {YELLOW}python tensortune_install.py{ENDC}
     (The installer is also updated for v1.1.1)

  2. {BOLD}Launch TensorTune:{ENDC}
     - {BOLD}GUI Version:{ENDC} {YELLOW}python tensortune_gui.py{ENDC} (or {GREEN}./launch_gui.sh{ENDC} or {GREEN}launch_tensortune_gui.bat{ENDC})
     - {BOLD}CLI Version:{ENDC} {YELLOW}python tensortune_cli.py{ENDC} (or {GREEN}./launch_cli.sh{ENDC} or {GREEN}launch_tensortune_cli.bat{ENDC})
...
if __name__ == "__main__":
    try:
        import shutil # Moved import here to ensure it's available before use in get_system_info
    except ImportError:
